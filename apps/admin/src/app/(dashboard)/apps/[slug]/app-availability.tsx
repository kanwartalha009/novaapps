...
/**
 * App-side availability manager (ADR-011), wired to the API.
 * PRIVATE = allowlist; PUBLIC = available to all except excluded. "Available" toggles per agency;
 * entries are computed on save (ALLOW for private, DENY for public exclusions).
 */
export function AppAvailability({
  ...
  function save() {
    ...
    startTransition(async () => {
      const res = await setAvailability("APP", appId, { mode, entries });
      if (!res.ok) {
  ...
  return (
    <Card className="p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-zinc-500">
            Which agencies may distribute this app. They see it only once the app is published <em>and</em> available here.
          </p>
        </div>
        <div className="flex shrink-0 gap-1 rounded-lg bg-zinc-100 p-0.5">
          {(["PRIVATE", "PUBLIC"] as const).map((m) => (
            <button
              key={m}
              onClick={() => setMode(m)}
              className={`rounded-md px-3 py-1.5 text-xs font-medium ${mode === m ? "bg-white text-zinc-900" : "text-zinc-500"}`}
            >
      ...
      <div className="mt-3 flex gap-2">
        <button onClick={() => setAvail(new Set(agencies.map((a) => a.id)))} className="text-xs font-semibold text-brand-600 hover:underline">
          Make all available
        </button>
        <span className="text-zinc-300">·</span>
        <button onClick={() => setAvail(new Set())} className="text-xs font-semibold text-zinc-500 hover:underline">
          Clear
        </button>
      </div>
            ...
            <li key={a.id} className="flex items-center justify-between gap-3 py-2.5">
              ...
              {on ? (
                <div className="flex items-center gap-2">
                  <Badge value="ACTIVE" />
                  <button onClick={() => toggle(a.id)} className="text-xs font-medium text-zinc-400 hover:text-danger-600">
                    {mode === "PUBLIC" ? "Exclude" : "Remove"}
                  </button>
                </div>
              ) : (
                <button onClick={() => toggle(a.id)} className="text-xs font-semibold text-brand-600 hover:underline">
                  Make available
                </button>
              )}
            </li>
